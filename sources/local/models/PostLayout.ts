...
export const PostLayout: Model = {
	...
	label: 'Post',
	labelField: 'title',
	filePath: 'content/pages/event/{slug}.md',
	fieldGroups: [
		{
			name: 'thumbnail',
			label: 'Thumbnail',
	...
	fields: [
		{
			...
			label: 'Title',
			required: true,
			default: 'This is an event title',
			hidden: false,
			localized: false
		},
		...
		},
		{
			type: 'boolean',
			name: 'isFeatured',
			label: 'Exclude from event feed',
			description: "Enable this option to avoid a featured post (in a 'Featured posts' section) to appear duplicated in the event page.",
			required: false,
			default: false,
			hidden: false,
			localized: false,
